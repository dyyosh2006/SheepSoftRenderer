...
Model::~Model()
{
	...
	if (smoothmap) delete smoothmap; smoothmap = NULL;
...
void Model::create_map(const char* filename)
{
	...
	texfile = texfile.substr(0, dot) + std::string("_smoothness.tga");
	if (_access(texfile.data(), 0) != -1)
	{
		smoothmap = new TGAImage();
		load_texture(filename, "_smoothness.tga", smoothmap);
	}
...
float Model::smoothness(vec2 uv)
{
	uv[0] = fmod(uv[0], 1);
	uv[1] = fmod(uv[1], 1);
	int uv0 = uv[0] * smoothmap->get_width();
	int uv1 = uv[1] * smoothmap->get_height();
	return smoothmap->get(uv0, uv1)[0] / 255.f;
}
